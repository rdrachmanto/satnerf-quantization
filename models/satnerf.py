...
def inference_trt(runner, args, rays_xyz, z_vals, rays_d=None, sun_d=None, rays_t=None):
    """
    ...
    # if rays_xyz is not None:
    #     print("rays_xyz shape:", rays_xyz.shape)
    #     xyz_ = rays_xyz.view(-1, 3)
    #     print("xyz_ shape:", xyz_.shape)
    # else:
    #     print("rays_xyz is None")
    ...
    out = runner.infer(feed_dict={
        "input_xyz": xyz_.cpu().numpy(),
        "input_sun_dir": sun_d_.cpu().numpy(),
        "input_t": rays_t_.cpu().numpy()
    })

    out = torch.from_numpy(out["output"]).to("cuda:0")

    #
    # print("[satnerf.inference:49] Inference time: ", (time.time() - start)*1000)
...
def inference(model, args, rays_xyz, z_vals, rays_d=None, sun_d=None, rays_t=None):
    ...
    # print(f"xyz_: {xyz_.shape}")
    # print(f"rays_d_: {None if rays_d_ is None else rays_d_.shape}")
    # print(f"sun_d_: {sun_d_.shape}")
    # print(f"input_t_: {rays_t_.shape}")
...
class SatNeRF(torch.nn.Module):
    ...
    def forward(self, input_xyz, input_direction=None, input_sun_direction=None, input_t=None, sigma_only=False):
        """
        ...
        xyz_ = input_xyz
        # print("[satnerf.SatNeRF.forward:189] xyz_.shape: ", xyz_.shape)
        ...
        # print("[satnerf.SatNeRF.forward:194] sigma_only: ", sigma_only)
        ...
        # print("[satnerf.SatNeRF.forward:201] input_direction: ", input_direction)
        ...
        # print("[satnerf.SatNeRF.forward:217] input_t.shape: ", input_t.shape)
